"""
Test script: download 001.mov from Google Drive and publish to Facebook Page.
...
log = logging.getLogger("test_facebook")
...
def main() -> None:
    log.info("=== test_facebook.py | SAFE_MODE=%s ===", config.SAFE_MODE)
    ...
    try:
        platform_meta = metadata.get_platform_data(VIDEO_ID, "facebook")
    except KeyError as exc:
        log.error("Metadata error: %s", exc)
        sys.exit(1)

    if platform_meta is None:
        log.error("No 'facebook' key in metadata for video '%s'.", VIDEO_ID)
        sys.exit(1)

    log.info("Metadata: %s", platform_meta)

    # 3. Publish to Facebook
    log.info("Step 3: publishing to Facebook...")
    result = facebook.publish(video_path, platform_meta)

    # 4. Print result
    print()
    if result["ok"]:
        if config.SAFE_MODE:
            print("✅ SAFE_MODE — would have published to Facebook (no actual request made).")
        else:
