"""
Test script: download 001.mov from Google Drive and publish to YouTube.
...
log = logging.getLogger("test_youtube")
...
def main() -> None:
    log.info("=== test_youtube.py | SAFE_MODE=%s ===", config.SAFE_MODE)
    ...
    try:
        platform_meta = metadata.get_platform_data(VIDEO_ID, "youtube")
    except KeyError as exc:
        log.error("Metadata error: %s", exc)
        sys.exit(1)

    if platform_meta is None:
        log.error("No 'youtube' key in metadata for video '%s'.", VIDEO_ID)
        sys.exit(1)

    log.info("Metadata: %s", platform_meta)

    # 3. Publish to YouTube
    log.info("Step 3: publishing to YouTube...")
    result = youtube.publish(video_path, platform_meta)

    # 4. Print result
    print()
    if result["ok"]:
        if config.SAFE_MODE:
            print("✅ SAFE_MODE — would have uploaded to YouTube (no actual request made).")
        else:
            print(f"✅ Uploaded! video_id={result['video_id']}")
    else:
