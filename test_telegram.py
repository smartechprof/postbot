"""
Test script: download 001.mov from Google Drive and publish to Telegram.

Environment variables are loaded from /etc/igbot.env if the file exists,
otherwise the system environment is used as-is.

Run with SAFE_MODE=1 (default in config.py) to skip actual publishing.
Set SAFE_MODE=0 in the env file to do a real send.
"""
...
log = logging.getLogger("test_telegram")
...
def main() -> None:
    log.info("=== test_telegram.py | SAFE_MODE=%s ===", config.SAFE_MODE)
    ...
    try:
        platform_meta = metadata.get_platform_data(VIDEO_ID, "telegram")
    except KeyError as exc:
        log.error("Metadata error: %s", exc)
        sys.exit(1)

    if platform_meta is None:
        log.error("No 'telegram' key in metadata for video '%s'.", VIDEO_ID)
        sys.exit(1)

    log.info("Metadata: %s", platform_meta)

    # 3. Publish to Telegram
    log.info("Step 3: publishing to Telegram...")
    result = telegram.publish(video_path, platform_meta)

    # 4. Print result
    print()
    if result["ok"]:
        if config.SAFE_MODE:
            print("✅ SAFE_MODE — would have sent to Telegram (no actual request made).")
        else:
